// Same browser history reader for the local Electron desktop side.
// Mirrors agent/history.js so This PC can show its own browser history too.
...
function chromiumProfiles(userDataDir) {
  ...
  let entries; try { entries = fs.readdirSync(userDataDir); } catch (e) { return []; }
  const profiles = [];
...
async function readChromium(file, limit) {
  const SQL = await ensureSql();
  const tmp = copyToTemp(file);
  let db = null;
  try {
    db = new SQL.Database(fs.readFileSync(tmp));
    const res = db.exec(`SELECT url, title, visit_count, last_visit_time FROM urls WHERE last_visit_time > 0 ORDER BY last_visit_time DESC LIMIT ${Math.max(1, Math.min(limit, 1000))}`);
    if (!res || !res[0]) return [];
    const EPOCH_DIFF_MS = 11644473600000;
    return res[0].values.map(([url, title, vc, t]) => ({
      url, title: title || url, visit_count: vc || 0,
...
async function readFirefox(file, limit) {
  const SQL = await ensureSql();
  const tmp = copyToTemp(file);
  let db = null;
  try {
    db = new SQL.Database(fs.readFileSync(tmp));
    const res = db.exec(`SELECT url, title, visit_count, last_visit_date FROM moz_places WHERE last_visit_date IS NOT NULL ORDER BY last_visit_date DESC LIMIT ${Math.max(1, Math.min(limit, 1000))}`);
    if (!res || !res[0]) return [];
    return res[0].values.map(([url, title, vc, t]) => ({
      url, title: title || url, visit_count: vc || 0,
...
async function getBrowserHistory({ icon, limit = 200 }) {
  ...
  if (CHROMIUM_BROWSERS.includes(ic)) {
    ...
    for (const p of profiles) {
      try {
        const entries = await readChromium(p.file, limit);
  ...
  if (FIREFOX_BROWSERS.includes(ic)) {
    ...
    for (const p of profiles) {
      try {
        const entries = await readFirefox(p.file, limit);
        for (const e of entries) all.push({ ...e, profile: p.name });
      } catch (err) {}
    }
    all.sort((a, b) => b.last_visit - a.last_visit);
  ...
  throw new Error(`History reading not supported for "${ic}"`);
}
