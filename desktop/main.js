...
// Map of well-known browser identifiers -> friendly name
const BROWSER_KEYS = {
...
function detectWindows() {
  ...
  // 1) Registry: HKLM and HKCU StartMenuInternet
  const regPaths = [
      ...
      for (const sub of subkeys) {
        try {
          ...
          found.push({
            ...
            path: exe,
            icon: detectIcon(rawName),
          });
        } catch (e) { /* skip */ }
      }
    } catch (e) { /* registry path missing */ }
  }

  // 2) Common install paths fallback
  const commonPaths = [
  ...
  for (const c of commonPaths) {
    if (fs.existsSync(c.p)) {
      found.push({ name: c.name, path: c.p, icon: detectIcon(c.name) });
    }
  }
...
function detectMac() {
  ...
  // Also Safari is always there on macOS
  if (!found.find(b => b.name === 'Safari')) {
  ...
  for (const c of candidates) {
    try {
      ...
      if (out && fs.existsSync(out)) {
        found.push({ name: c.name, path: out, icon: detectIcon(c.name) });
      }
    } catch (e) { /* not found */ }
  }
  return uniqueByName(found);
...
function openInBrowser(browserPath, url) {
  ...
  // Normalize URL
  let final = url.trim();
  if (!/^https?:\/\//i.test(final) && !/^file:\/\//i.test(final)) {
    final = 'https://' + final;
  }

  const platform = process.platform;

  // macOS: prefer `open -a` with .app bundle
  if (platform === 'darwin' && browserPath.includes('.app')) {
    const appBundle = browserPath.split('.app')[0] + '.app';
    spawn('open', ['-a', appBundle, final], { detached: true, stdio: 'ignore' }).unref();
    return final;
  }

  // Windows + Linux + macOS direct binary
...
function createWindow() {
  ...
  // Open external links in OS default browser instead of Electron
...
ipcMain.handle('detect-browsers', async () => {
  try {
    ...
    return { ok: true, browsers: list, platform: process.platform };
  } catch (e) {
    return { ok: false, error: e.message, browsers: [], platform: process.platform };
  }
});

ipcMain.handle('open-url', async (_evt, { browserPath, url }) => {
  try {
    const final = openInBrowser(browserPath, url);
